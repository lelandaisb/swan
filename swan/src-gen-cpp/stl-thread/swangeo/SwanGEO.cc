...
void
SwanGEO::Options::jsonInit(const rapidjson::Value::ConstObject& d)
{
	// outputPath
...
SwanGEO::SwanGEO(const Options& aOptions, SwanGEOFunctions& aSwanGEOFunctions)
: options(aOptions)
, swanGEOFunctions(aSwanGEOFunctions)
, t_n(0.0)
, t_nplus1(0.0)
...
void SwanGEO::initDijini() noexcept
{
	...
	else
	{
		...
		for (size_t icInnerCells=0; icInnerCells<nbInnerCells; icInnerCells++)
		{
			const Id icId(innerCells[icInnerCells]);
			const size_t icCells(icId);
			Dijini[icCells] = swanGEOFunctions.nextDepth();
		}
	}
}
...
{
	{
		...
		for (size_t icInnerCells=0; icInnerCells<nbInnerCells; icInnerCells++)
		{
			const Id icId(innerCells[icInnerCells]);
			const size_t icCells(icId);
			Hini[icCells] = swanGEOFunctions.nextWaveHeight();
		}
	}
}
...
{
	{
		...
		for (size_t icInnerCells=0; icInnerCells<nbInnerCells; icInnerCells++)
		{
			const Id icId(innerCells[icInnerCells]);
			const size_t icCells(icId);
			Xc[icCells][0] = swanGEOFunctions.nextLon();
			Xc[icCells][1] = swanGEOFunctions.nextLat();
		}
	}
}
...
void SwanGEO::initdeltaxdeltay() noexcept
{
	...
	}
	{
		...
		parallel::parallel_exec(nbOuterCells, [&](const size_t& ocOuterCells)
		{
...
int main(int argc, char* argv[]) 
{
	...
	else
	{
		...
		std::cerr << "(SwanGEODefaultOptions.json)" << std::endl;
		return -1;
	}
	...
	if (d.HasMember("swanGEOFunctions"))
	{
		const rapidjson::Value& valueof_swanGEOFunctions = d["swanGEOFunctions"];
		assert(valueof_swanGEOFunctions.IsObject());
		swanGEOFunctions.jsonInit(valueof_swanGEOFunctions.GetObject());
	}
	
	// simulator must be a pointer if there is a finalize at the end (Kokkos, omp...)
	auto simulator = new SwanGEO(options, swanGEOFunctions);
	simulator->simulate();
